...
class MultiQueryOutput(BaseModel):
    queries: list[str] = Field(description="List of alternative query formulations and closely related concepts.")
...
def generate_multi_queries(query: str, metadata: str = "") -> list[str]:
    ...
    prompt_template = ChatPromptTemplate.from_messages(
        ...
        - Alternative Phrasing: Generate variations that preserve the core engineering intent but utilize different technical vocabulary or syntax.
        - Neighboring Concepts: Include variations targeting concepts immediately adjacent or critical to the user's topic.
         make sure the query generated revolves aeound the metadat passed dont keep any query empty
         Collection Context:
        {metadata}
    ...
    response = query_generation_chain.invoke({"query": query, "metadata": metadata})
